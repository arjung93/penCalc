...
It is possible that subscribers increase their contributions to the
NPS during their working life from the mandatory 20\%
contributions. We therefore show an example of a contribution rate
that is 20\% for the first five years, increases by 2\% every five
years, and is 30\% for the last 11 years of working life. The
remaining assumptions remain the same as that of the life-cycle
portfolio allocation example described in Section
\ref{ss:lifecycleN}. The R-code is shown below. We first set the vector
of contributions in \texttt{cont\_vector}.

<<ex5>>=
cont_vector<- c(rep(0.20,5), rep(0.22,5),rep(0.24,5),
                rep(0.26,5),rep(0.28,5), rep(0.30,11))

set.seed(111)
# 40% annuity
x <- pencalc(wage=list(25000
                 ,0.05
                 ,cont_vector
                 ,initial.amount=0))
x
